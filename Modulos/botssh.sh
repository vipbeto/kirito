...
fun_botOnOff() {
    [[ $(ps x | grep "bot_plus" | grep -v grep | wc -l) = '0' ]] && {
        clear
        echo -e "\E[44;1;37m             INSTALADOR BOT BETO_VPN                \E[0m\n"
        ...
        read iduser
        clear
        echo -e "\033[1;32mINICIANDO BOT BETO_VPN \033[0m\n"
        fun_bot1() {
            [[ ! -e "/etc/BETO_VPN/ShellBot.sh" ]] && wget https://raw.githubusercontent.com/vipbeto/kirito/main/ShellBot.sh -O /etc/BETO_VPN/ShellBot.sh
            cd /etc/BETO_VPN
			rm -R bot
			wget https://raw.githubusercontent.com/vipbeto/kirito/main/Modulos/bot
			chmod 777 bot
            screen -dmS bot_plus ./bot $tokenbot $iduser >/dev/null 2>&1
            [[ $(grep -wc "bot_plus" /etc/autostart) = '0' ]] && {
                echo -e "ps x | grep 'bot_plus' | grep -v 'grep' || cd /etc/BETO_VPN && sudo screen -dmS bot_plus ./bot $tokenbot $iduser && cd $HOME" >>/etc/autostart
            } || {
        ...
        fun_bar 'fun_bot2'
        echo -e "\n\033[1;32m \033[1;31mBOT BETO_VPN PARADO! \033[0m"
        sleep 2
        menu
    }
}
...
[[ -f "/etc/BETO_VPN/ShellBot.sh" ]] && fun_botOnOff || fun_instbot
#fim

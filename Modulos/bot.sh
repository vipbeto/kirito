...
fun_menu() {
    if [[ "${message_from_id[$id]}" = "$id_admin" ]]; then
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>SEJA BEM VINDO(a) AO BOT CrashARM</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
...
fun_ajuda() {
    ...
    if [[ "$id_chatuser" = "$id_admin" ]]; then
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>BEM VINDO(a) AO BOT CrashARM</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
...
criar_user() {
    ...
    echo "$senha" >/etc/CrashARM/senha/$usuario
...
fun_del_user() {
    ...
    [[ "${message_from_id[$id]}" = "$id_admin" ]] && {
        ...
        rm /etc/CrashARM/senha/$usuario >/dev/null 2>&1
    } || {
    ...
    [[ -e /etc/CrashARM/userteste/$usuario.sh ]] && rm /etc/CrashARM/userteste/$usuario.sh
    ...
    if [[ "${callback_query_from_id[$id]}" = "$id_admin" ]]; then
        ...
        fun_infu() {
            ...
            for user in $(cat /etc/passwd | awk -F : '$3 >= 1000 {print $1}' | grep -v nobody); do
                info='===========================\n'
                [[ -e /etc/CrashARM/senha/$user ]] && senha=$(cat /etc/CrashARM/senha/$user) || senha='Null'
...
sobremim() {
    local msg
    msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
    msg+="<b>🤖 BOT CrashARM MANAGER 🤖</b>\n"
    msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
    msg+="<b>Desenvolvido por:</b> @openersfamily\n"
    msg+="<b>Canal Oficial:</b> @CrashARM\n\n"
...
fun_teste() {
    ...
    cat <<-EOF >/etc/CrashARM/userteste/$usuario.sh
	#!/bin/bash
	# USUARIO TESTE
	...
	rm /etc/CrashARM/senha/$usuario > /dev/null 2>&1
	rm /etc/CrashARM/userteste/$usuario.sh
	EOF
    chmod +x /etc/CrashARM/userteste/$usuario.sh
    echo "/etc/CrashARM/userteste/$usuario.sh" | at now + $t_time hour >/dev/null 2>&1
...
fun_exp_user() {
    if [[ "${callback_query_from_id[$id]}" == "$id_admin" ]]; then
        ...
        for user in $(cat /etc/passwd | awk -F : '$3 >= 1000 {print $1}' | grep -v nobody); do
            ...
            [[ -e /etc/CrashARM/userteste/$user.sh ]] && rm /etc/CrashARM/userteste/$user.sh
...
fun_backauto() {
    [[ "${callback_query_from_id[$id]}" = "$id_admin" ]] && {
        [[ ! -d /etc/CrashARM/backups ]] && {
            mkdir /etc/CrashARM/backups
            ...
            [[ -d /etc/CrashARM/backups ]] && rm -rf /etc/CrashARM/backups
            ShellBot.answerCallbackQuery --callback_query_id ${callback_query_id[$id]} \
...
backup_auto() {
    ShellBot.sendDocument --chat_id $id_admin \
        --document "@/etc/CrashARM/backups/backup.vps" \
        --caption "$(echo -e "♻️ BACKUP AUTOMATICO ♻️")"
    rm /etc/CrashARM/backups/backup.vps
    return 0
}
...
while :; do
    [[ -e "/etc/CrashARM/backups/backup.vps" ]] && {
        backup_auto
    }

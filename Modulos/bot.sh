...
#   DESENVOLVIDO POR: betovip
#   API SHELLBOT: SHAMAN
#   CONTATO TELEGRAM: http://t.me/betovip
...
fun_menu() {
    [[ "${message_from_id[$id]}" == "$id_admin" ]] && {
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>SEJA BEM VINDO BOT BETOVIP X</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
    ...
    if [[ "$(grep -w "${message_from_username}" $ativos | grep -wc 'revenda')" != '0' ]]; then
        ...
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>SEJA BEM VINDO BOT ANUBIS X</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
        env_msg+="⚠️ <i>SELECIONE UMA OPCAO ABAIXO !</i>\n\n"
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} --text "$env_msg" \
...
fun_ajuda() {
    ...
    if [[ "$id_chatuser" = "$id_admin" ]]; then
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>BEM VINDO BOT ANUBIS X</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
...
criar_user() {
    ...
    vl=$(sed -n '5 p' $file_user | cut -d' ' -f2)
    ...
    [[ "${message_from_id[$id]}" != "$id_admin" ]] && {
        echo "$usuario:$senha:$info_data:$limite:$vl" >/etc/bot/revenda/${message_from_username}/usuarios/$usuario
    ...
    echo "usuario $usuario $validade $senha $vl criado"
}
...
sobremim() {
    ...
    msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
    msg+="<b>Desenvolvido por:</b> @oogeniohacker\n"
...
fun_del_rev() {
    ...
    [[ "${callback_query_from_id[$id]}" == "$id_admin" ]] || [[ "$(grep -wc ${callback_query_from_username} $ativos)" != '0' ]] && {
        ShellBot.sendMessage --chat_id ${callback_query_message_chat_id[$id]} \
            --text "🗑 REMOVER REVENDEDOR 🗑\n\nInforme o user dele [Ex: @oogeniohacker]:" \
            --reply_markup "$(ShellBot.ForceReply)"
    } || {
        ShellBot.answerCallbackQuery --callback_query_id ${callback_query_id[$id]} \
            --text "🚫 ACESSO NEGADO 🚫"
        ...
        (
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                    ...
                    [[ "$Resp" = @(Sim|sim|SIM) ]] && {
                        ...
                        [[ -e /tmp/$filebkp ]] && {
                            [[ -e "/etc/openvpn" ]] && {
                                ...
                                ShellBot.sendMessage --chat_id ${id_admin} \
                                    --text "$(echo -e "✅ Cerficiado Ovpn restaurado\ncom sucesso!")" \
                                    --parse_mode html
                                ShellBot.sendMessage --chat_id ${id_admin} \
                                    --text "$(echo -e "♻️ OPENVPN REINICIADO!")" \
                                    --parse_mode html
                                ShellBot.sendMessage --chat_id ${id_admin} \
                ...
                '👥 ADICIONAR REVENDEDOR 👥\n\nInforme o nome:')
                    ...
                    echo "Nome: ${message_text[$id]}" >$CAD_ARQ
                    ShellBot.sendMessage --chat_id ${message_from_id[$id]} \
                        --text 'Informe o user dele [Ex: @oogeniohacker]:' \
                        --reply_markup "$(ShellBot.ForceReply)"
                    ;;
                'Informe o user dele [Ex: @oogeniohacker]:')
                    verifica_acesso
                    ...
                    [[ -z $_VAR1 ]] && {
                        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                            --text "$(echo -e "❌ Erro \n\n⚠️ Informe o user [EX: @oogeniohacker]")" \
                            --parse_mode html
                        break
                    }

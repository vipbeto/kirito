...
[[ -z $api_bot ]] && exit 0
[[ -z $id_admin ]] && exit 0
[[ $(awk -F" " '{print $2}' /usr/lib/licence) != "@BETO_SSH" ]] && exit 0
...
fun_menu() {
    if [[ "${message_from_id[$id]}" = "$id_admin" ]]; then
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>SEJA BEM VINDO(a) AO BOT BETO_VPN</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
...
fun_ajuda() {
    ...
    if [[ "$id_chatuser" = "$id_admin" ]]; then
        local env_msg
        env_msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
        env_msg+="<b>BEM VINDO(a) AO BOT BETO_VPN</b>\n"
        env_msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
...
criar_user() {
    ...
    echo "$senha" >/etc/BETO_VPN/senha/$usuario
...
fun_del_user() {
    ...
    [[ "${message_from_id[$id]}" = "$id_admin" ]] && {
        ...
        rm /etc/BETO_VPN/senha/$usuario >/dev/null 2>&1
    } || {
    ...
    [[ -e /etc/BETO_VPN/userteste/$usuario.sh ]] && rm /etc/BETO_VPN/userteste/$usuario.sh
    ...
    if [[ "${callback_query_from_id[$id]}" = "$id_admin" ]]; then
        ...
        fun_infu() {
            ...
            for user in $(cat /etc/passwd | awk -F : '$3 >= 1000 {print $1}' | grep -v nobody); do
                info='===========================\n'
                [[ -e /etc/BETO_VPN/senha/$user ]] && senha=$(cat /etc/BETO_VPN/senha/$user) || senha='Null'
...
sobremim() {
    local msg
    msg="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n"
    msg+="<b>🤖 BOT BETO_VPN MANAGER 🤖</b>\n"
    msg+="=×=×=×=×=×=×=×=×=×=×=×=×=×=\n\n"
    msg+="<b>Desenvolvido por:</b> @openersfamily\n"
    msg+="<b>Canal Oficial:</b> @BETO_VPN\n\n"
...
fun_teste() {
    ...
    cat <<-EOF >/etc/BETO_VPN/userteste/$usuario.sh
	#!/bin/bash
	# USUARIO TESTE
	...
	rm /etc/BETO_VPN/senha/$usuario > /dev/null 2>&1
	rm /etc/BETO_VPN/userteste/$usuario.sh
	EOF
    chmod +x /etc/BETO_VPN/userteste/$usuario.sh
    echo "/etc/BETO_VPN/userteste/$usuario.sh" | at now + $t_time hour >/dev/null 2>&1
...
fun_exp_user() {
    if [[ "${callback_query_from_id[$id]}" == "$id_admin" ]]; then
        ...
        for user in $(cat /etc/passwd | awk -F : '$3 >= 1000 {print $1}' | grep -v nobody); do
            ...
            [[ -e /etc/BETO_VPN/userteste/$user.sh ]] && rm /etc/BETO_VPN/userteste/$user.sh
...
fun_backauto() {
    [[ "${callback_query_from_id[$id]}" = "$id_admin" ]] && {
        [[ ! -d /etc/BETO_VPN/backups ]] && {
            mkdir /etc/BETO_VPN/backups
            ...
            [[ -d /etc/BETO_VPN/backups ]] && rm -rf /etc/BETO_VPN/backups
            ShellBot.answerCallbackQuery --callback_query_id ${callback_query_id[$id]} \
...
backup_auto() {
    ShellBot.sendDocument --chat_id $id_admin \
        --document "@/etc/BETO_VPN/backups/backup.vps" \
        --caption "$(echo -e "♻️ BACKUP AUTOMATICO ♻️")"
    rm /etc/BETO_VPN/backups/backup.vps
    return 0
}
...
while :; do
    [[ -e "/etc/BETO_VPN/backups/backup.vps" ]] && {
        backup_auto
    }
